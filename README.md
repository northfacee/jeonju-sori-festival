...
5. **summarize_trip** — 완성된 일정을 보고 코스 이름과 추천 이유를 생성합니다.
...
`kind`는 축제 공연이면 `null`이고, 앱이 추가한 일정이면 `food` · `cafe` · `night-tour` 중 하나입니다.
